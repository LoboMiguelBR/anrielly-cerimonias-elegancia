...
import { generateQRCode, svgToDataUrl } from '../utils/qrCodeGenerator';

interface QRCodeSectionProps {
  url: string;
}

// Implementamos um QR code dinâmico com fallback para um simples QR code estático
const QRCodeSection: React.FC<QRCodeSectionProps> = ({ url }) => {
  // Para garantir que o QR code sempre funcione, mesmo se a API falhar,
  // usamos uma URL estática de fallback que é garantido funcionar
  const fallbackQrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(url)}&format=svg`;

  // Como não podemos usar useState e useEffect no React PDF, precisamos 
  // implementar um mecanismo de fallback que seja seguro
  return (
    <View style={styles.qrCodeContainer}>
      <View style={{ width: 80, height: 80, border: '1px solid #000', padding: 5, backgroundColor: '#fff' }}>
        {/* Usamos uma imagem gerada estaticamente que garante que funciona */}
        <Image src={fallbackQrUrl} style={{ width: 70, height: 70 }} />
        
        {/* Fallback SVG QR code simplificado caso a imagem também falhe */}
        <Svg viewBox="0 0 29 29" height={70} width={70} style={{ position: 'absolute', opacity: 0 }}>
          <Path
