...
const QRCodeSection: React.FC<QRCodeSectionProps> = ({ url }) => {
  // URL da API do QR Code Generator
  // Em ambiente de produção, esta chamada deveria ser realizada em um edge function
  // para proteger o token de acesso e evitar o uso excessivo da API
  const qrCodeUrl = `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(url)}&format=svg`;

  return (
    <View style={styles.qrCodeContainer}>
      <View style={{ width: 80, height: 80, border: '1px solid #000', padding: 5, backgroundColor: '#fff' }}>
        {/* Usamos uma imagem gerada dinamicamente */}
        <Image src={qrCodeUrl} style={{ width: 70, height: 70 }} />
        
        {/* Fallback SVG QR code simplificado caso a imagem dinâmica falhe */}
