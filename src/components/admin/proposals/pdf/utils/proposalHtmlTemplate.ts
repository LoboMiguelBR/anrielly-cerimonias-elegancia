...
import { replaceProposalVariables } from '../../variables/proposalVariables';
...
export const generateProposalHTML = (
  ...
  if (template && template.html_content) {
    ...
    const defaultTemplate = getDefaultProposalTemplate();
    htmlContent = defaultTemplate.html;
    cssContent = defaultTemplate.css;
  }

  // Replace variables in both HTML and CSS
  const processedHtml = replaceProposalVariables(htmlContent, proposal);
  const processedCss = replaceProposalVariables(cssContent, proposal);
  ...
  const finalHtml = `
    ...
    <head>
        ...
        <title>Proposta - ${proposal.client_name}</title>
        <style>
            ${processedCss}
        </style>
    </head>
...
const getDefaultProposalTemplate = () => {
  const html = `
