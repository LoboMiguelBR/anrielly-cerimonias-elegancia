  ...
  try {
    ...
    if (action === "register") {
      ...
      const { data, error } = await supabase
        ...
        .select("id, nome_responsavel, email, status")
        .maybeSingle();
      ...
      return new Response(JSON.stringify({ questionario: data }), { headers: corsHeaders });
    }
    ...
    if (action === "login") {
      ...
      const { data, error } = await supabase
        .from("questionario_acessos")
        .select("id, nome_responsavel, email, senha_hash, status")
        .eq("email", email)
        .eq("link_publico", linkPublico)
        .maybeSingle();
      if (error) {
      ...
      const resp = { ...data };
      delete resp.senha_hash;
      return new Response(JSON.stringify({ questionario: resp }), { headers: corsHeaders });
    }
